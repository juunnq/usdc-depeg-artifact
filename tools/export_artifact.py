...
Steps (see the four top-level functions below, run in order by main()):
  ...
  3. verify_reproduction(), run the documented five-step sequence + `make fc27`
...
def copy_whitelist(out: Path) -> tuple[int, list[tuple[Path, int]]]:
    ...
    fc27_src = REPO_ROOT / "paper" / "fc27"
    fc27_dst = out / "paper" / "fc27"
    _copy_tree(fc27_src, fc27_dst, file_count, large_files)

    # The short paper is a SEPARATE submission built from the same analysis package, not
    # a mode of the regular one, and it is the variant actually submitted. An export that
    # carried only paper/fc27/ would hand a reviewer following the short paper's own
    # artifact URL the files for the other paper.
    short_src = REPO_ROOT / "paper" / "fc27short"
    if short_src.is_dir():
        _copy_tree(short_src, out / "paper" / "fc27short", file_count, large_files)

    fc27_dst_makefile_dir = out / "paper"
    fc27_dst_makefile_dir.mkdir(parents=True, exist_ok=True)
    _write_trimmed_makefile(fc27_dst_makefile_dir)
    file_count[0] += 1
...
KNOWN_BENIGN_HITS: dict[tuple[str, int], str] = {
    ("paper/fc27/refs.bib", 519): "cited author's public ORCID, not this paper's own",
...
def sweep_pdf(out: Path) -> list[str]:
    ...
    hand."""
    pdf_path = out / "paper" / "fc27" / "fc27.pdf"
    if not pdf_path.exists():
        return ["fc27.pdf not present in export to check (expected after step 3)"]
    try:
        import fitz  # PyMuPDF
    except ImportError:
...
def verify_reproduction(out: Path) -> dict:
    """Run the documented five-step sequence, then `make fc27`, inside the export,
    ...
    steps = [
        ...
        (["python", "redemptions.py", "--from-frozen"], None),
    ...
    # make fc27, try `make`, fall back to `mingw32-make` (this build machine has
    # only the latter; a reviewer's machine may have either).
    paper_dir = out / "paper"
    make_bin = shutil.which("make") or shutil.which("mingw32-make")
    if not make_bin:
        raise AssertionError("neither `make` nor `mingw32-make` found on PATH")
    r = _run([make_bin, "fc27"], paper_dir, env)
    if r.returncode != 0:
        raise AssertionError(f"`{make_bin} fc27` failed:\nSTDOUT:\n{r.stdout[-3000:]}\n"
                              f"STDERR:\n{r.stderr[-3000:]}")

    pdf_path = out / "paper" / "fc27" / "fc27.pdf"
    report["pdf_sha256"] = hashlib.sha256(pdf_path.read_bytes()).hexdigest()

    # The SHORT target is the submission, so the export must prove IT rebuilds, not only
    # the regular one. The Makefile carries a single trimmed fc27 target, so the short
    # build is driven the way the export's README documents it.
    if (out / "paper" / "fc27short" / "fc27short.tex").exists():
...
def _artifact_hash_diff(out: Path) -> dict:
    ...
    groups = [
        ...
        (REPO_ROOT / "paper" / "fc27" / "tables", out / "paper" / "fc27" / "tables", "*.tex"),
    ]
...
def main() -> int:
    ...
    if not args.skip_repro:
        ...
        if src_failed:
        ...
        elif src_n != repro_report.get("pytest_passed") + repro_report.get("pytest_skipped", 0):
            print(f"      WARNING: export passed {repro_report.get('pytest_passed')} "
                  ...
                  f"source repo passes {src_n}, reconcile before "
            ...
            print(f"      test count reconciles with the source repo: "
                  ...
                  f"{repro_report['pytest_skipped']} skipped = {src_n}. Skips here are "
                  f"tests that need a built paper, which the sequence builds afterwards.")
        else:
            print(f"      test count matches the source repo: {src_n}")
        print(f"      artifact hashes: {diff['agree']} agree / {diff['differ']} differ / "
              f"{diff['missing']} missing")
        for status, name in diff["details"]:
            print(f"        {status} {name}")
        print(f"      regular PDF SHA-256: {repro_report.get('pdf_sha256')}")
        print(f"      SHORT PDF SHA-256  : {repro_report.get('short_pdf_sha256')} "
              f"<- the submission")
        if CHECKLIST.exists():
            checklist_text = CHECKLIST.read_text(encoding="utf-8")
            for label, key in (("regular", "pdf_sha256"), ("SHORT", "short_pdf_sha256")):
    ...
    if repro_report:
        print(f"  pytest: {repro_report.get('pytest_passed')} passed")
        print(f"  PDF SHA-256: {repro_report.get('pdf_sha256')}")
