...
aggregating ~14,111 senders, so unwinding it only lowers the HHI.

**Upper bound (burn-attributed, N=776 redeemers):**
- raw HHI: 0.2558   normalized HHI*: 0.2548   (effective redeemers ~3.9)
- top-10 share: 88.1%   largest: 40.9%
- seeded bootstrap 95% CI: 0.0517-0.4184  (wide -- hinges on the Coinbase conduits)
- dominant entity: Coinbase = 69.6% via 2 conduits (hot wallet 0xa9d1e08c77...)

**With that one verified custodian removed:** raw HHI 0.0671, normalized 0.0659,
largest 16.8% (effective redeemers ~14.9, N=774).
=> economic-redeemer HHI <= 0.26, and ~0.07 once the one custodian is unwound.

- coverage: 90% of $8.15B gross burns (FIFO burn-attributed; rest mint-funded / pre-window balance)
- MZZ check: consistent -- low, custodial-routed concentration; matches MZZ low USDC arbitrage centralization (~521 arbitrageurs) vs USDT (~6)
- entity-attributed HHI: not computed -- no MZZ/Arkham/Nansen label set in repo; the bound + verified Coinbase mapping is the complete result
