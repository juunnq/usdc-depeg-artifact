...
def test_ordering_null_claim_extraction_actually_quotes_the_claims():
    ...
    assert "most fragile" in claims["ex_ante_claim"]
    assert "USDT (no SVB exposure) least" in claims["ex_ante_claim"]
    assert claims["ex_ante_claim_tex_line"] > 0
    assert "Realized ordering" in claims["realized_ordering_claim"]
