...
fc27.tex claims a FULL rank order, not merely "USDC is most fragile." The cross-coin
...
PSM, which is what Appendix B establishes when it declines to read DAI's break as an
...
module hardcoded lines[521:524] / lines[589:591], pinned to the fc27.tex line numbers
...
FC27_TEX = Path(__file__).resolve().parent.parent / "paper" / "fc27" / "fc27.tex"
...
def enumerate_permutations() -> list:
    ...
    what Appendix B establishes, so the manuscript cannot treat DAI as exchangeable here
...
def _extract_between(norm: str, start_anchor: str, end_anchor: str) -> str:
    ...
    if i < 0:
        raise ValueError(f"ordering_null.py: start anchor not found in fc27.tex: "
                          f"{start_anchor!r}")
    j = norm.find(end_anchor, i)
    if j < 0:
        raise ValueError(f"ordering_null.py: end anchor not found after its start "
                          f"anchor in fc27.tex: {end_anchor!r}")
...
def _manuscript_claim_lines() -> dict:
    ...
    Reads the \\input-expanded manuscript, not fc27.tex alone. G5b split the body into
    sections/*.tex and fc27.tex became a skeleton, at which point both anchors below
    ...
    raw = manuscript_source()
    norm = re.sub(r"\s+", " ", raw)

    exante_start = "A global-games run model"
    realized_start = r"\paragraph{Realized ordering.}"
    exante = _extract_between(norm, exante_start, "and USDT (no SVB exposure) least.")
    ...
    realized = _extract_between(norm, realized_start,
                                "matching the ex-ante fragility ordering")
    return {
...
def report() -> dict:
    ...
    return {
        ...
        "claim_type_determination": (
            ...
            f"(fc27.tex:{claims['ex_ante_claim_tex_line']}) states an explicit 3-way "
            "ranking (USDC most fragile, DAI next, USDT least) and the realized-ordering "
            f"paragraph (fc27.tex:{claims['realized_ordering_claim_tex_line']}) reports "
            "the realized troughs as consistent with that ex-ante exposure ranking. DAI "
            "is mechanically linked to USDC through the PSM (Appendix B), so it is not an "
        ...
        "note": (
            ...
            "linked USDC-DAI pair, because Appendix B establishes DAI's break runs "
